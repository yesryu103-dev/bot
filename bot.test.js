...
test("trade USD enrichment prefers Dexscreener spot price", () => {
  ...
  assert.ok(Math.abs(priced.execPriceUsd - (7.5 * 1784) / 83073.3186) < 0.001);
  assert.equal(priced.spotPriceUsd, 0.1597);
  assert.equal(priced.priceUsd, priced.execPriceUsd);
...
test("v4 swap log decoder uses user-perspective signs (pay ETH = BUY)", () => {
  ...
  // V4: negative ETH = user pays ETH; positive token = user receives token → BUY
  const buy = bot.tradeFromV4SwapLog({
    amount0: -(15n * 10n ** 17n),
  ...
  assert.equal(buy.quoteAmount, 1.5);
  assert.equal(buy.dexVer, "v4");

  // V4: positive ETH = user receives ETH; negative token = user pays token → SELL
  const sell = bot.tradeFromV4SwapLog({
    amount0: 1134287337412999641n,
    amount1: -603932412960000000000000n,
    key,
    baseToken: token,
    txHash: "0x8eae",
    blockNumber: 23957256,
    timestampMs: Date.now(),
    sender: "0x39b38686a19836ac10162c490e4558e120cbbe5f",
  });
  assert.ok(sell);
  assert.equal(sell.side, "SELL");
  assert.ok(Math.abs(sell.quoteAmount - 1.1342873374129996) < 1e-9);
...
test("portfolio keeps liquid tokens and hides junk", () => {
  ...
  assert.equal(portfolio.bagItems.some((item) => item.symbol === "SCAM"), false);
